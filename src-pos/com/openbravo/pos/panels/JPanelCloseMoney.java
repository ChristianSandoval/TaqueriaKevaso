        ...
        if (res == JOptionPane.YES_OPTION) {
            ...
            try {
                ...
                if (m_App.getActiveCashDateEnd() == null) {
                    ...
                    props.put("mail.smtp.user", "conectiva@live.com.mx");
                    props.put("mail.smtp.password", "1default1");
                    props.put("mail.smtp.port", "587");
                    AppConfig config = new AppConfig(new File(System.getProperty("user.home") + File.separator + "openbravopos.properties"));
                    config.load();
                    Session session = Session.getInstance(props, new GMailAuthenticator("conectiva@live.com.mx", "1default1"));
                    try {
                        Message message = new MimeMessage(session);
                        message.setFrom(new InternetAddress("conectiva@live.com.mx"));
                        ...
                        for (int i = 0; i < productSales.size(); i++) {
                            body = body + "<tr><td>" + ((PaymentsModel.ProductSalesLine) productSales.get(i)).printProductName() + "</td><td>" + ((PaymentsModel.ProductSalesLine) productSales.get(i)).getProductUnits() + "</td><td>&nbsp;&nbsp;&nbsp;" + ((PaymentsModel.ProductSalesLine) productSales.get(i)).printTotal() + "</td><td>" + ((PaymentsModel.ProductSalesLine) productSales.get(i)).printGanancias() + "</td></tr>";
                        }
                        ...
                        transport.connect("smtp.office365.com", 587, "conectiva@live.com.mx", "1default1");
                        message.saveChanges();
