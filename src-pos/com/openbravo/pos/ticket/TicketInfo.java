...
public class TicketInfo implements SerializableRead, Externalizable {
    ...
    public List<TicketLineInfo> getLinesBarra() {
        List<TicketLineInfo> l = new ArrayList<TicketLineInfo>();
        for(int i=0;i<m_aLines.size();i++)
        {
            if(m_aLines.get(i).getProperty("bebida", "").equals("bebida")
                    &&m_aLines.get(i).getProperty("impreso", "NO").equals("NO"))
            {
                l.add(m_aLines.get(i));
            }
        }
        return l;
    }
    public List<TicketLineInfo> getLinesCocina() {
        List<TicketLineInfo> l = new ArrayList<TicketLineInfo>();
        for(int i=0;i<m_aLines.size();i++)
        {
            if(m_aLines.get(i).getProperty("cocina", "").equals("cocina")
                    &&m_aLines.get(i).getProperty("impreso", "NO").equals("NO"))
            {
                l.add(m_aLines.get(i));
            }
