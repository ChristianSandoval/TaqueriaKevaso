...
public class DataLogicSales extends BeanFactoryDataSingle {
    ...
    throws BasicException
    {
        Object result = new PreparedSentence(this.s, "SELECT S.USER FROM SHAREDTICKETS S INNER JOIN PLACES P ON P.ID=S.ID  WHERE P.NAME = ?", SerializerWriteString.INSTANCE, SerializerReadString.INSTANCE).find(name);
    ...
    public final UserInfo getUserById(String id) throws BasicException {
        return (UserInfo) new PreparedSentence(s
            , "SELECT ID, NAME FROM PEOPLE WHERE ID=?"
            , SerializerWriteString.INSTANCE
            , UserInfo.getSerializerRead()).find(id);
    }
    // Listados para combo
    ...
    public final void saveTicket(final TicketInfo ticket, final Place place,final String location) throws BasicException {
        ...
        Transaction t = new Transaction(s) {
            public Object transact() throws BasicException {
                ...
                // new ticket
                new PreparedSentence(s
                    , "INSERT INTO TICKETS (ID, TICKETTYPE, TICKETID, PERSON, CUSTOMER, FLOOR, PLACE) VALUES (?, ?, ?, ?, ?,?,?)"
                    , SerializerWriteParams.INSTANCE
                    ).exec(new DataParams() { public void writeValues() throws BasicException {
                        setString(1, ticket.getId());
                        ...
                        setString(6, place.getFloor());
                        setString(7, place.getId());
                    }});
                ...
                for (TicketLineInfo l : ticket.getLines()) {
                    ...
                    if (l.getProductID() != null)  {
                        ...
                        while (count < properties.size())
                        {
                          ...
                          DataLogicSales.this.getStockDiaryInsert().exec(new Object[] { UUID.randomUUID().toString(), ticket.getDate(), MovementReason.OUT_SALE.getKey(), location, key, null, new Double(-Double.parseDouble(element) * l.getMultiply()), new Double(productInfo.getPriceBuy()) });
