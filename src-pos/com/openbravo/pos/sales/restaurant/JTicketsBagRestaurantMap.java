...
//    private static final Icon ICO_FRE = new NullIcon(22, 22);
    ...
    public JTicketsBagRestaurantMap(AppView app, TicketsEditor panelticket) {
        ...
        try {
            SentenceList sent = new StaticSentence(
                    app.getSession(), 
                    "SELECT P.ID, P.NAME, P.X, P.Y, P.FLOOR, F.NAME AS NAMEFLOOR FROM PLACES P INNER JOIN FLOORS F ON P.FLOOR=F.ID ORDER BY P.FLOOR", 
                    null, 
    ...
    private void setActivePlace(Place place, TicketInfo ticket) {
        ...
        m_panelticket.setActiveTicket(ticket, place);//m_PlaceCurrent.getName());
            ...
            if (m_PlaceClipboard == null) {  
                ...
                if (customer == null) {
                    ...
                    if (ticket == null && !m_place.hasPeople()) {
                        ...
                        if(!m_App.getAppUserView().getUser().hasPermission("meserear"))
                        {
                            JOptionPane.showMessageDialog(null, "No tienes permisos para abrir mesas");
                            return;
                        }
                        // table occupied
                        ticket = new TicketInfo();
                        try {
                            dlReceipts.insertSharedTicket(m_place.getId(), ticket, m_App.getAppUserView().getUser().getUserInfo().getId());
                        } catch (BasicException e) {
                            new MessageInf(e).show(JTicketsBagRestaurantMap.this); // Glup. But It was empty.
                        }
                        m_place.setPeople(true);
                        //if(!m_place.getFloor().equals("0")){
                            //m_place.setName(JOptionPane.showInputDialog("Nombre:"));
                        //}
                        ...
                        else
                        {
                            Object resp;
                            try {
                                resp = new PreparedSentence(m_App.getSession()
                                        , "select user from sharedtickets where id=?"
                                        , SerializerWriteString.INSTANCE, SerializerReadString.INSTANCE).find(m_place.getId());

                                if(resp!=null)
                                {
                                    String user = m_App.getAppUserView().getUser().getId();
                                    if(String.valueOf(resp).equals(user)){
                                        setActivePlace(m_place, ticket);  
                                    }
                                    else
                                    {
                                        JOptionPane.showMessageDialog(null, "Esta mesa le corresponde a otro mesero");
                ...
                if (ticketclip == null) {
                    ...
                    if (m_PlaceClipboard == m_place) {
                        ...
                        if (ticket == null) {
                            try {
                                dlReceipts.insertSharedTicket(m_place.getId(), ticketclip, m_App.getAppUserView().getUser().getUserInfo().getId());
